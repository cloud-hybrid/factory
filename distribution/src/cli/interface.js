...
const Main = async () => {
    ...
    const Arguments = async () => {
        const Commands = {
            ...
            secrets: {
                "get-secret": async (input) => await API.Get(input)
            }
        };
        return await CLI(Process.argv.splice(2))
            ...
            .command("secrets-manager", "Secrets Management Interface", ( async ($) => {
                $.hide("version");
                ...
                $.command("get", "Index AWS Secret from Secrets-Manager Service", ( async ($) => await Commands.secrets[ "get-secret" ]($) ));
            } ))
            /*** String Manipulation */
