 ...
 *
 */
function Copy(source, target, debug = false) {
    ...
    FS.readdirSync(source, { withFileTypes: true }).filter(($) => ( !Exclusions.includes($.name) )).forEach(($) => {
        ...
        if ( File ) {
            try {
                FS.copyFileSync(Path.join(source, $.name), Path.join(target, $.name), FS.constants.COPYFILE_FICLONE);
