...
class Construct {
    /*** Target Deployable Lambda Layers Check */
    layers?: boolean;

    /*** Target Deployable Lambda Service Folder Check */
    service?: boolean;

    /*** Target Deployable Lambda `artifacts.json` Check */
    artifacts?: boolean;

    private static readonly packages: string = Packages;

    constructor() {
        this.artifacts = Construct.includes("artifacts.json");
        this.service = Construct.includes("service");
        this.layers = Construct.includes("layers");
    }
...
const Implementation: typeof Construct = new Proxy(Construct, {
    apply (target: Construct | any, type, parameters: any[]) {
        return target(... parameters);
    }
});

interface Implementation extends Construct {}
const Interface: () => Implementation = () => {
